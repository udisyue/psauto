...
cotime = time.strftime('%Y-%m-%d',time.localtime(time.time()+86400*31))
cur_time = re.sub(r'-', "", citime)

info = readDict.readManualDis(324763)
# set param
in_param = {}
...
# wrong pare checkin and checkout
...
case_name = readDict.selfName()
rlt = detail(in_param_json)
jrlt = json.loads(rlt)

if jrlt['service_status']['msg'] == 'outer_param_err' :
    print ("######## %s success"%case_name)
else :
    print ("######## %s failed\n"%case_name)
    print ("######## msg = %s"%jrlt['service_status']['msg'])
