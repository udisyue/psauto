...
writeDict.autoPriceInfo_write(0, 1, 1)
...
if offset != 0 :
    ...
    jrlt = json.loads(rlt)
    if jrlt['service_status']['msg'] == 'success' and jrlt['list_response']['list_hotel']['list_ota_list'][0]['hotel_status']==2:
        b_s_price = jrlt['list_response']['list_hotel']['list_ota_list'][0]['room_list'][0]['product_list'][0]['base_sale_price']['total_price']['amount']
        ori_price = jrlt['list_response']['list_hotel']['list_ota_list'][0]['room_list'][0]['product_list'][0]['origin_price']['total_price']['amount']
        ota = jrlt['list_response']['list_hotel']['list_ota_list'][0]['ota_id']
        # mbl parse
        base = jrlt['list_response']['list_hotel']['list_ota_list'][0]['base_price']['total_price']['amount']
        cost = jrlt['list_response']['list_hotel']['list_ota_list'][0]['room_list'][0]['product_list'][0]['cost_price']['total_price']['amount']
        rate = (base - cost) / base
        b = float(rule[3])
        if b > 1 :
            ...
            if modify_price < base :
                if b_s_price == modify_base:
                    print '#####base sale price = modify base price, auto price success'
                else:
                    print '#####base sale price != modify baes price, auto price failed'